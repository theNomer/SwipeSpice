...
class Recipe(db.Model):
    ...
    image = db.Column(db.LargeBinary, nullable=True)  # Stores image as binary data (BLOB)
    image_url = db.Column(db.String(500), nullable=True)  # Optional for file paths or URLs
    ingredients = db.Column(db.Text, nullable=False)
    instructions = db.Column(db.Text, nullable=False)
    allergies = db.Column(db.ARRAY(db.String), nullable=True)  # Array of allergens
    diet_type = db.Column(db.String(80), nullable=True)  # E.g., vegetarian, vegan
    ...
    comments = db.relationship('Comment', backref='recipe', lazy=True)  # Relationship to comments
